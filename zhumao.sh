...
installChatGPT(){
    ...
    docker run --name zhumao-chatgpt -e PANDORA_CLOUD=cloud -e PANDORA_SERVER=0.0.0.0:33366 -p 33366:33366 -d pengzhile/pandora
    echoContent yellow " ---> chatgpt安装完成"
    echoContent yellow " ---> 访问此处即可使用: " && echoContent green "$(getPublicIP):33366"
}
...
installNPM(){
  ...
  jc21/nginx-proxy-manager:latest
    ...
    echoContent yellow " ---> 访问此处即可使用: " && echoContent green "$(getPublicIP):81"
...
menu() {
    ...
    echoContent lightYellow "当前版本：v1.0.4"
    ...
    echoContent yellow "(8) 安装chatgpt"
